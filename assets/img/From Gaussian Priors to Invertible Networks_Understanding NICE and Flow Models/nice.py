...
class Shuffle(Layer):
    """Shuffle layer, provide two ways to shuffle input dimension:
    one is to reverse, the other is to shuffle randomly, default is to reverse dimension.
    """
...
class SplitVector(Layer):
    """
    Split the input into two parts, interleave the partitions
    """
...
class ConcatVector(Layer):
    """
    Concatenate the two partitions into one vector
    """
    def __init__(self, **kwargs):
...
class AddCouple(Layer):
    """
    AddCouple layer, provide two ways to add or subtract two parts of input
    """
